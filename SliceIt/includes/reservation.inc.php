...
require 'dbh.inc.php';
    ...
    if(empty($fname) || empty($lname) || empty($date) || empty($time) || empty($guests) || empty($tele)) {
        ...
        header("Location: ../reservation.php?error3=invalidcomments");
        exit();
    }
    ...
    if($current_tables + $tables > $a_tables){
        header("Location: ../reservation.php?error3=full");
         ...
         $sql = "INSERT INTO reservation(f_name, l_name, num_guests, num_tables, rdate, time_zone, telephone, comments, user_fk) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
